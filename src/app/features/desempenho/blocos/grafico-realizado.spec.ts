...
describe('Gráfico de resultado realizado por ativo', () => {
  ...
  it('@spec:AC-202 cada ticker apurado aparece com o valor, e o total exibido é o do servidor', async () => {
    const elemento = await montar([{ ticker: 'VALE3', valor: 600 }, { ticker: 'MGLU3', valor: -100 }], 500);

    const tickers = [...elemento.querySelectorAll('[data-realizado-ticker]')].map((no) =>
      no.textContent?.trim(),
    );
    expect(tickers).toEqual(['VALE3', 'MGLU3']);
    expect(elemento.querySelector('[data-realizado="MGLU3"] [data-realizado-valor]')?.textContent)
      .toContain('100,00');
  ...
  it('@spec:AC-202 perda apurada se distingue do ganho sem depender de cor', async () => {
    ...
    expect(elemento.querySelector('[data-realizado="VALE3"]')?.getAttribute('data-direcao')).toBe('alta');
    expect(elemento.querySelector('[data-realizado="MGLU3"]')?.getAttribute('data-direcao')).toBe('baixa');
