#!/usr/bin/env bash
# executar-tarefas.sh — gerado por `onp-spec plano repaginacao-acesso` em 2026-09-09 17:53
...
#
# resumo do que está rolando, a qualquer momento: onp-spec resumo repaginacao-acesso
set -u
set -o pipefail

RUN_ID='api-externa-frontend-v2-repaginacao-acesso-mtuecpx9'
FEATURE='repaginacao-acesso'
BASE_BRANCH='spec/repaginacao-acesso'
...
preparar_ambiente() {
  ...
  git ls-files --error-unmatch -- '.spec/features/repaginacao-acesso/spec.md' >/dev/null 2>&1 || falhar "spec.md não está commitada — os worktrees das faixas precisam dela no git"
  ...
  git worktree prune
  LOG_DIR="$(dirname "$TOPLEVEL")/onp-worktrees/api-externa-frontend-v2-repaginacao-acesso-logs"
  WT_BASE="$(dirname "$TOPLEVEL")/onp-worktrees/api-externa-frontend-v2-repaginacao-acesso"
...
mesclar_faixa() { # $1=faixa $2=branch $3=worktree $4=exit-da-faixa
  if [ "$4" -ne 0 ]; then
    ...
    amarelo "  reexecute só ela: bash .spec/features/repaginacao-acesso/executar-tarefas.sh --faixa $1"
...
executar_faixa_1() {
  ...
  preparar_worktree 'faixa-1' 'spec/repaginacao-acesso-faixa-1' "$WT" || return 1
  evento --tipo faixa --faixa 'faixa-1' --estado executando --tentativa "$(tentativa 'faixa-1')"
  : > "$LOG_DIR/faixa-1.log"
  (
    cd "$WT" || exit 9
    rodar_tarefa 'faixa-1' 'T-103' 'Você executa UMA tarefa da feature "repaginacao-acesso" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/repaginacao-acesso/spec.md, .spec/features/repaginacao-acesso/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-103 — "Moldura da tela de acesso"
  critérios/refs: AC-251 (Tela de acesso em duas regiões), AC-252 (A região de marca não atrapalha quem usa teclado), AC-253 (Movimento na marca é opcional)
  arquivos permitidos (e seus testes): src/app/features/acesso/moldura-acesso.ts, src/app/features/acesso/moldura-acesso.html, src/app/features/acesso/moldura-acesso.scss, src/app/features/acesso/moldura-acesso.spec.ts
  mensagem de commit: "T-103 repaginacao-acesso: Moldura da tela de acesso"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'claude-sonnet-5' high
  ...
  local st=$?
  mesclar_faixa 'faixa-1' 'spec/repaginacao-acesso-faixa-1' "$WT" "$st" || return 1
  marcar_concluidas T-103
  return 0
}
...
executar_faixa_2() {
  ...
  preparar_worktree 'faixa-2' 'spec/repaginacao-acesso-faixa-2' "$WT" || return 1
  evento --tipo faixa --faixa 'faixa-2' --estado executando --tentativa "$(tentativa 'faixa-2')"
  : > "$LOG_DIR/faixa-2.log"
  (
    cd "$WT" || exit 9
    rodar_tarefa 'faixa-2' 'T-104' 'Você executa UMA tarefa da feature "repaginacao-acesso" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/repaginacao-acesso/spec.md, .spec/features/repaginacao-acesso/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-104 — "Medidor de força de senha"
  critérios/refs: AC-255 (O medidor de senha reflete a política real)
  arquivos permitidos (e seus testes): src/app/features/acesso/forca-da-senha.ts, src/app/features/acesso/forca-da-senha.html, src/app/features/acesso/forca-da-senha.scss, src/app/features/acesso/forca-da-senha.spec.ts
  mensagem de commit: "T-104 repaginacao-acesso: Medidor de força de senha"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'claude-sonnet-5' medium
  ...
  local st=$?
  mesclar_faixa 'faixa-2' 'spec/repaginacao-acesso-faixa-2' "$WT" "$st" || return 1
  marcar_concluidas T-104
  return 0
}
...
executar_faixa_3() {
  ...
  preparar_worktree 'faixa-3' 'spec/repaginacao-acesso-faixa-3' "$WT" || return 1
  evento --tipo faixa --faixa 'faixa-3' --estado executando --tentativa "$(tentativa 'faixa-3')"
  : > "$LOG_DIR/faixa-3.log"
  (
    cd "$WT" || exit 9
    rodar_tarefa 'faixa-3' 'T-105' 'Você executa UMA tarefa da feature "repaginacao-acesso" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/repaginacao-acesso/spec.md, .spec/features/repaginacao-acesso/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-105 — "Tela de login"
  critérios/refs: AC-251 (Tela de acesso em duas regiões), AC-254 (Erro aparece depois da interação e diz como corrigir), AC-256 (O primeiro campo já está pronto para digitar), AC-257 (Botão mostra que está enviando sem perder o rótulo), AC-258 (Falha de acesso é comunicada no nível certo)
  arquivos permitidos (e seus testes): src/app/features/acesso/login/login.ts, src/app/features/acesso/login/login.html, src/app/features/acesso/login/login.scss, src/app/features/acesso/login/login.spec.ts
  mensagem de commit: "T-105 repaginacao-acesso: Tela de login"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'claude-sonnet-5' high
  ...
  local st=$?
  mesclar_faixa 'faixa-3' 'spec/repaginacao-acesso-faixa-3' "$WT" "$st" || return 1
  marcar_concluidas T-105
  return 0
}
...
executar_faixa_4() {
  ...
  preparar_worktree 'faixa-4' 'spec/repaginacao-acesso-faixa-4' "$WT" || return 1
  evento --tipo faixa --faixa 'faixa-4' --estado executando --tentativa "$(tentativa 'faixa-4')"
  : > "$LOG_DIR/faixa-4.log"
  (
    cd "$WT" || exit 9
    rodar_tarefa 'faixa-4' 'T-106' 'Você executa UMA tarefa da feature "repaginacao-acesso" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/repaginacao-acesso/spec.md, .spec/features/repaginacao-acesso/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-106 — "Tela de cadastro"
  critérios/refs: AC-251 (Tela de acesso em duas regiões), AC-254 (Erro aparece depois da interação e diz como corrigir), AC-255 (O medidor de senha reflete a política real), AC-256 (O primeiro campo já está pronto para digitar), AC-257 (Botão mostra que está enviando sem perder o rótulo), AC-258 (Falha de acesso é comunicada no nível certo)
  arquivos permitidos (e seus testes): src/app/features/acesso/cadastro/cadastro.ts, src/app/features/acesso/cadastro/cadastro.html, src/app/features/acesso/cadastro/cadastro.scss, src/app/features/acesso/cadastro/cadastro.spec.ts
  mensagem de commit: "T-106 repaginacao-acesso: Tela de cadastro"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'claude-sonnet-5' high
  ...
  local st=$?
  mesclar_faixa 'faixa-4' 'spec/repaginacao-acesso-faixa-4' "$WT" "$st" || return 1
  marcar_concluidas T-106
  return 0
}
...
encerrar() { # $1=escopo
  ...
  if [ "$COM_GATE" -eq 0 ]; then
    ...
    if [ -z "$FALHAS" ]; then
      ...
      amarelo "  para o veredito: bash .spec/features/repaginacao-acesso/executar-tarefas.sh --gate"
      exit 0
    fi
...
listar() {
  ...
  echo "  faixa-1  onda 1  T-103"
  echo "  faixa-2  onda 1  T-104"
  echo "  faixa-3  onda 1  T-105"
  echo "  faixa-4  onda 2  T-106"
  echo
