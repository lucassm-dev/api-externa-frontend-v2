#!/usr/bin/env bash
# executar-tarefas.sh — gerado por `onp-spec plano fundacao` em 2026-09-09 00:11
...
RUN_ID='api-externa-frontend-v2-fundacao-mttcfk37'
FEATURE='fundacao'
...
preparar_ambiente() {
  ...
  git worktree prune
  LOG_DIR="$(dirname "$TOPLEVEL")/onp-worktrees/api-externa-frontend-v2-fundacao-logs"
  WT_BASE="$(dirname "$TOPLEVEL")/onp-worktrees/api-externa-frontend-v2-fundacao"
...
T-001 — "Projeto Angular, Material, Vitest, proxy e estrutura de pastas"
  critérios/refs: AC-024 (O navegador enxerga a API na mesma origem)
  arquivos permitidos (e seus testes): angular.json, package.json, tsconfig.json, tsconfig.app.json, tsconfig.spec.json, proxy.conf.json, onpspec.config.json, .gitignore, src/main.ts, src/index.html, src/app/app.ts, src/app/app.html, src/app/app.config.ts, src/app/app.routes.ts, src/app/features/LEIA-ME.md
  mensagem de commit: "T-001 fundacao: Projeto Angular, Material, Vitest, proxy e estrutura de pastas"
...
executar_faixa_2() {
  ...
  (
    cd "$WT" || exit 9
    rodar_tarefa 'faixa-2' 'T-002' 'Você executa UMA tarefa da feature "fundacao" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/fundacao/spec.md, .spec/features/fundacao/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-002 — "Locale pt-BR, moeda e data"
  critérios/refs: AC-019 (Real e dólar saem formatados em pt-BR e distinguíveis), AC-020 (Data e hora seguem a convenção brasileira)
  arquivos permitidos (e seus testes): src/app/core/formatacao/formatacao.ts, src/app/core/formatacao/formatacao.spec.ts
  mensagem de commit: "T-002 fundacao: Locale pt-BR, moeda e data"
...
executar_faixa_3() {
  ...
  (
    cd "$WT" || exit 9
    rodar_tarefa 'faixa-3' 'T-003' 'Você executa UMA tarefa da feature "fundacao" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/fundacao/spec.md, .spec/features/fundacao/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-003 — "Tokens de cor e alternância de tema"
  critérios/refs: AC-022 (A escolha de tema é aplicada e lembrada)
  arquivos permitidos (e seus testes): src/styles.scss, src/styles/_tokens.scss, src/styles/_tema.scss, src/app/core/tema/tema.service.ts, src/app/core/tema/tema.service.spec.ts
  mensagem de commit: "T-003 fundacao: Tokens de cor e alternância de tema"
...
executar_faixa_4() {
  ...
  (
    cd "$WT" || exit 9
    rodar_tarefa 'faixa-4' 'T-004' 'Você executa UMA tarefa da feature "fundacao" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/fundacao/spec.md, .spec/features/fundacao/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-004 — "Tipos do contrato da API"
  critérios/refs: AC-023 (Resposta paginada do Spring é lida pelos campos dela)
  arquivos permitidos (e seus testes): src/app/core/api/pagina.ts, src/app/core/api/erro-padrao.ts, src/app/core/api/api.spec.ts
  mensagem de commit: "T-004 fundacao: Tipos do contrato da API"
...
executar_faixa_5() {
  ...
  (
    cd "$WT" || exit 9
    rodar_tarefa 'faixa-5' 'T-005' 'Você executa UMA tarefa da feature "fundacao" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/fundacao/spec.md, .spec/features/fundacao/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-005 — "Sessão: modelo, armazenamento e estado"
  critérios/refs: AC-011 (A sessão sobrevive ao fechamento do navegador), AC-012 (Sair apaga a sessão), AC-013 (Token vencido não é sessão), AC-014 (O aviso de sessão acabando sai do próprio login)
  arquivos permitidos (e seus testes): src/app/core/sessao/sessao.model.ts, src/app/core/sessao/sessao.service.ts, src/app/core/sessao/sessao.service.spec.ts
  mensagem de commit: "T-005 fundacao: Sessão: modelo, armazenamento e estado"
...
executar_faixa_6() {
  ...
  preparar_worktree 'faixa-6' 'spec/fundacao-faixa-6' "$WT" || return 1
  evento --tipo faixa --faixa 'faixa-6' --estado executando --tentativa "$(tentativa 'faixa-6')"
  : > "$LOG_DIR/faixa-6.log"
  (
    cd "$WT" || exit 9
    rodar_tarefa 'faixa-6' 'T-006' 'Você executa UMA tarefa da feature "fundacao" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/fundacao/spec.md, .spec/features/fundacao/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-006 — "Interceptor que envia o token"
  critérios/refs: AC-017 (O token vai no cabeçalho Authorization), AC-018 (Sem sessão, nada de cabeçalho)
  arquivos permitidos (e seus testes): src/app/core/sessao/autenticacao.interceptor.ts, src/app/core/sessao/autenticacao.interceptor.spec.ts
  mensagem de commit: "T-006 fundacao: Interceptor que envia o token"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'claude-sonnet-5' medium
  ...
  local st=$?
  mesclar_faixa 'faixa-6' 'spec/fundacao-faixa-6' "$WT" "$st" || return 1
...
executar_faixa_7() {
  ...
  preparar_worktree 'faixa-7' 'spec/fundacao-faixa-7' "$WT" || return 1
  evento --tipo faixa --faixa 'faixa-7' --estado executando --tentativa "$(tentativa 'faixa-7')"
  : > "$LOG_DIR/faixa-7.log"
  (
    cd "$WT" || exit 9
    rodar_tarefa 'faixa-7' 'T-007' 'Você executa UMA tarefa da feature "fundacao" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/fundacao/spec.md, .spec/features/fundacao/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-007 — "Guarda de rota das telas internas"
  critérios/refs: AC-015 (Sem sessão, a tela interna não renderiza), AC-016 (Com sessão, a tela interna libera)
  arquivos permitidos (e seus testes): src/app/core/sessao/sessao.guard.ts, src/app/core/sessao/sessao.guard.spec.ts
  mensagem de commit: "T-007 fundacao: Guarda de rota das telas internas"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'claude-sonnet-5' medium
  ...
  local st=$?
  mesclar_faixa 'faixa-7' 'spec/fundacao-faixa-7' "$WT" "$st" || return 1
...
executar_faixa_8() {
  ...
  preparar_worktree 'faixa-8' 'spec/fundacao-faixa-8' "$WT" || return 1
  evento --tipo faixa --faixa 'faixa-8' --estado executando --tentativa "$(tentativa 'faixa-8')"
  : > "$LOG_DIR/faixa-8.log"
  (
    cd "$WT" || exit 9
    rodar_tarefa 'faixa-8' 'T-008' 'Você executa UMA tarefa da feature "fundacao" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/fundacao/spec.md, .spec/features/fundacao/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-008 — "Catálogo de erros e tradutor por código"
  critérios/refs: AC-001 (Código conhecido vira a mensagem do produto), AC-002 (Código desconhecido cai no genérico com o código visível), AC-003 (Erro de validação aponta os campos, nunca um balão único), AC-004 (Falta de resposta do servidor tem mensagem própria)
  arquivos permitidos (e seus testes): src/app/core/erros/catalogo-erros.ts, src/app/core/erros/tradutor-erro.ts, src/app/core/erros/tradutor-erro.spec.ts
  mensagem de commit: "T-008 fundacao: Catálogo de erros e tradutor por código"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'claude-sonnet-5' medium
  ...
  local st=$?
  mesclar_faixa 'faixa-8' 'spec/fundacao-faixa-8' "$WT" "$st" || return 1
...
executar_faixa_9() {
  ...
  preparar_worktree 'faixa-9' 'spec/fundacao-faixa-9' "$WT" || return 1
  evento --tipo faixa --faixa 'faixa-9' --estado executando --tentativa "$(tentativa 'faixa-9')"
  : > "$LOG_DIR/faixa-9.log"
  (
    cd "$WT" || exit 9
    rodar_tarefa 'faixa-9' 'T-009' 'Você executa UMA tarefa da feature "fundacao" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/fundacao/spec.md, .spec/features/fundacao/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-009 — "Interceptor de erro que encerra a sessão"
  critérios/refs: AC-005 (Token inválido ou expirado encerra a sessão)
  arquivos permitidos (e seus testes): src/app/core/erros/erro.interceptor.ts, src/app/core/erros/erro.interceptor.spec.ts
  mensagem de commit: "T-009 fundacao: Interceptor de erro que encerra a sessão"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'claude-sonnet-5' medium
  ...
  local st=$?
  mesclar_faixa 'faixa-9' 'spec/fundacao-faixa-9' "$WT" "$st" || return 1
...
executar_faixa_10() {
  ...
  preparar_worktree 'faixa-10' 'spec/fundacao-faixa-10' "$WT" || return 1
  evento --tipo faixa --faixa 'faixa-10' --estado executando --tentativa "$(tentativa 'faixa-10')"
  : > "$LOG_DIR/faixa-10.log"
  (
    cd "$WT" || exit 9
    rodar_tarefa 'faixa-10' 'T-010' 'Você executa UMA tarefa da feature "fundacao" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/fundacao/spec.md, .spec/features/fundacao/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-010 — "Componentes de feedback dos três níveis"
  critérios/refs: AC-006 (Os três níveis se apresentam de forma inequivocamente distinta), AC-007 (Sucesso com aviso nunca é apresentado como falha)
  arquivos permitidos (e seus testes): src/app/core/feedback/feedback.model.ts, src/app/core/feedback/feedback.service.ts, src/app/core/feedback/mensagem-feedback.ts, src/app/core/feedback/mensagem-feedback.html, src/app/core/feedback/mensagem-feedback.scss, src/app/core/feedback/feedback.spec.ts
  mensagem de commit: "T-010 fundacao: Componentes de feedback dos três níveis"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'claude-sonnet-5' medium
  ...
  local st=$?
  mesclar_faixa 'faixa-10' 'spec/fundacao-faixa-10' "$WT" "$st" || return 1
...
executar_faixa_11() {
  ...
  preparar_worktree 'faixa-11' 'spec/fundacao-faixa-11' "$WT" || return 1
  evento --tipo faixa --faixa 'faixa-11' --estado executando --tentativa "$(tentativa 'faixa-11')"
  : > "$LOG_DIR/faixa-11.log"
  (
    cd "$WT" || exit 9
    rodar_tarefa 'faixa-11' 'T-011' 'Você executa UMA tarefa da feature "fundacao" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/fundacao/spec.md, .spec/features/fundacao/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-011 — "Valor com horário e marcação de dado defasado"
  critérios/refs: AC-008 (Todo preço aparece com o momento em que foi obtido), AC-009 (Dado com mais de 15 minutos ganha marcação visual), AC-010 (O limite de 15 minutos é exato)
  arquivos permitidos (e seus testes): src/app/core/dados/idade-dado.ts, src/app/shared/valor-com-horario/valor-com-horario.ts, src/app/shared/valor-com-horario/valor-com-horario.html, src/app/shared/valor-com-horario/valor-com-horario.scss, src/app/shared/valor-com-horario/valor-com-horario.spec.ts
  mensagem de commit: "T-011 fundacao: Valor com horário e marcação de dado defasado"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'claude-sonnet-5' medium
  ...
  local st=$?
  mesclar_faixa 'faixa-11' 'spec/fundacao-faixa-11' "$WT" "$st" || return 1
...
executar_faixa_12() {
  ...
  preparar_worktree 'faixa-12' 'spec/fundacao-faixa-12' "$WT" || return 1
  evento --tipo faixa --faixa 'faixa-12' --estado executando --tentativa "$(tentativa 'faixa-12')"
  : > "$LOG_DIR/faixa-12.log"
  (
    cd "$WT" || exit 9
    rodar_tarefa 'faixa-12' 'T-012' 'Você executa UMA tarefa da feature "fundacao" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/fundacao/spec.md, .spec/features/fundacao/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-012 — "Variação com sinal, sem depender de cor"
  critérios/refs: AC-021 (Variação positiva e negativa têm sinal próprio)
  arquivos permitidos (e seus testes): src/app/shared/variacao/variacao.ts, src/app/shared/variacao/variacao.html, src/app/shared/variacao/variacao.scss, src/app/shared/variacao/variacao.spec.ts
  mensagem de commit: "T-012 fundacao: Variação com sinal, sem depender de cor"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'claude-sonnet-5' medium
  ...
  local st=$?
  mesclar_faixa 'faixa-12' 'spec/fundacao-faixa-12' "$WT" "$st" || return 1
