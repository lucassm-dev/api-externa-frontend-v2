#!/usr/bin/env bash
# executar-tarefas.sh — gerado por `onp-spec plano repaginacao-desempenho` em 2026-09-09 17:53
...
#
# resumo do que está rolando, a qualquer momento: onp-spec resumo repaginacao-desempenho
set -u
set -o pipefail

RUN_ID='api-externa-frontend-v2-repaginacao-desempenho-mtuecpyp'
FEATURE='repaginacao-desempenho'
BASE_BRANCH='spec/repaginacao-desempenho'
...
preparar_ambiente() {
  ...
  git ls-files --error-unmatch -- '.spec/features/repaginacao-desempenho/spec.md' >/dev/null 2>&1 || falhar "spec.md não está commitada — os worktrees das faixas precisam dela no git"
  ...
  git worktree prune
  LOG_DIR="$(dirname "$TOPLEVEL")/onp-worktrees/api-externa-frontend-v2-repaginacao-desempenho-logs"
  WT_BASE="$(dirname "$TOPLEVEL")/onp-worktrees/api-externa-frontend-v2-repaginacao-desempenho"
...
mesclar_faixa() { # $1=faixa $2=branch $3=worktree $4=exit-da-faixa
  if [ "$4" -ne 0 ]; then
    ...
    amarelo "  reexecute só ela: bash .spec/features/repaginacao-desempenho/executar-tarefas.sh --faixa $1"
...
executar_faixa_1() {
  ...
  preparar_worktree 'faixa-1' 'spec/repaginacao-desempenho-faixa-1' "$WT" || return 1
  evento --tipo faixa --faixa 'faixa-1' --estado executando --tentativa "$(tentativa 'faixa-1')"
  : > "$LOG_DIR/faixa-1.log"
  (
    cd "$WT" || exit 9
    rodar_tarefa 'faixa-1' 'T-107' 'Você executa UMA tarefa da feature "repaginacao-desempenho" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/repaginacao-desempenho/spec.md, .spec/features/repaginacao-desempenho/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-107 — "Geometria da rosca"
  critérios/refs: AC-259 (Composição em rosca com rótulos legíveis)
  arquivos permitidos (e seus testes): src/app/features/desempenho/blocos/setores-da-rosca.ts, src/app/features/desempenho/blocos/setores-da-rosca.spec.ts
  mensagem de commit: "T-107 repaginacao-desempenho: Geometria da rosca"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'claude-sonnet-5' medium
  ...
  local st=$?
  mesclar_faixa 'faixa-1' 'spec/repaginacao-desempenho-faixa-1' "$WT" "$st" || return 1
  marcar_concluidas T-107
  return 0
}
...
executar_faixa_2() {
  ...
  preparar_worktree 'faixa-2' 'spec/repaginacao-desempenho-faixa-2' "$WT" || return 1
  evento --tipo faixa --faixa 'faixa-2' --estado executando --tentativa "$(tentativa 'faixa-2')"
  : > "$LOG_DIR/faixa-2.log"
  (
    cd "$WT" || exit 9
    rodar_tarefa 'faixa-2' 'T-108' 'Você executa UMA tarefa da feature "repaginacao-desempenho" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/repaginacao-desempenho/spec.md, .spec/features/repaginacao-desempenho/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-108 — "Composição em rosca"
  critérios/refs: AC-259 (Composição em rosca com rótulos legíveis), AC-260 (A cor nunca carrega a informação sozinha), AC-261 (Destacar uma fatia destaca sua legenda), AC-262 (A ressalva de soma continua visível)
  ...
  mensagem de commit: "T-108 repaginacao-desempenho: Composição em rosca"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'claude-sonnet-5' high
  ...
  local st=$?
  mesclar_faixa 'faixa-2' 'spec/repaginacao-desempenho-faixa-2' "$WT" "$st" || return 1
...
executar_faixa_3() {
  ...
  preparar_worktree 'faixa-3' 'spec/repaginacao-desempenho-faixa-3' "$WT" || return 1
  evento --tipo faixa --faixa 'faixa-3' --estado executando --tentativa "$(tentativa 'faixa-3')"
  : > "$LOG_DIR/faixa-3.log"
  (
    cd "$WT" || exit 9
    rodar_tarefa 'faixa-3' 'T-109' 'Você executa UMA tarefa da feature "repaginacao-desempenho" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/repaginacao-desempenho/spec.md, .spec/features/repaginacao-desempenho/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-109 — "Contribuição com linha de zero"
  critérios/refs: AC-263 (Contribuição com eixo comum e sinal explícito)
  ...
  mensagem de commit: "T-109 repaginacao-desempenho: Contribuição com linha de zero"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'claude-sonnet-5' medium
  ...
  local st=$?
  mesclar_faixa 'faixa-3' 'spec/repaginacao-desempenho-faixa-3' "$WT" "$st" || return 1
...
executar_faixa_4() {
  ...
  preparar_worktree 'faixa-4' 'spec/repaginacao-desempenho-faixa-4' "$WT" || return 1
  evento --tipo faixa --faixa 'faixa-4' --estado executando --tentativa "$(tentativa 'faixa-4')"
  : > "$LOG_DIR/faixa-4.log"
  (
    cd "$WT" || exit 9
    rodar_tarefa 'faixa-4' 'T-110' 'Você executa UMA tarefa da feature "repaginacao-desempenho" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/repaginacao-desempenho/spec.md, .spec/features/repaginacao-desempenho/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-110 — "Realizado por ticker"
  critérios/refs: AC-263 (Contribuição com eixo comum e sinal explícito)
  ...
  mensagem de commit: "T-110 repaginacao-desempenho: Realizado por ticker"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'claude-sonnet-5' medium
  ...
  local st=$?
  mesclar_faixa 'faixa-4' 'spec/repaginacao-desempenho-faixa-4' "$WT" "$st" || return 1
...
executar_faixa_5() {
  ...
  preparar_worktree 'faixa-5' 'spec/repaginacao-desempenho-faixa-5' "$WT" || return 1
  evento --tipo faixa --faixa 'faixa-5' --estado executando --tentativa "$(tentativa 'faixa-5')"
  : > "$LOG_DIR/faixa-5.log"
  (
    cd "$WT" || exit 9
    rodar_tarefa 'faixa-5' 'T-111' 'Você executa UMA tarefa da feature "repaginacao-desempenho" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/repaginacao-desempenho/spec.md, .spec/features/repaginacao-desempenho/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-111 — "Números do desempenho e composição da tela"
  critérios/refs: AC-264 (Indicador com rótulo, valor e selo), AC-265 (Nenhum bloco sugere histórico que não existe)
  arquivos permitidos (e seus testes): src/app/features/desempenho/blocos/numeros-desempenho.ts, src/app/features/desempenho/blocos/numeros-desempenho.html, src/app/features/desempenho/blocos/numeros-desempenho.scss, src/app/features/desempenho/desempenho.html, src/app/features/desempenho/desempenho.scss, src/app/features/desempenho/blocos/numeros-desempenho.spec.ts
  mensagem de commit: "T-111 repaginacao-desempenho: Números do desempenho e composição da tela"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'claude-sonnet-5' medium
  ...
  local st=$?
  mesclar_faixa 'faixa-5' 'spec/repaginacao-desempenho-faixa-5' "$WT" "$st" || return 1
...
encerrar() { # $1=escopo
  ...
  if [ "$COM_GATE" -eq 0 ]; then
    ...
    if [ -z "$FALHAS" ]; then
      ...
      amarelo "  para o veredito: bash .spec/features/repaginacao-desempenho/executar-tarefas.sh --gate"
      exit 0
    fi
...
listar() {
  ...
  echo "  faixa-1  onda 1  T-107"
  echo "  faixa-2  onda 1  T-108"
  echo "  faixa-3  onda 1  T-109"
  echo "  faixa-4  onda 2  T-110"
  echo "  faixa-5  onda 2  T-111"
  echo
