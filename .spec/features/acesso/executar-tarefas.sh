#!/usr/bin/env bash
# executar-tarefas.sh — gerado por `onp-spec plano acesso` em 2026-09-09 00:40
...
#
# resumo do que está rolando, a qualquer momento: onp-spec resumo acesso
set -u
set -o pipefail

RUN_ID='api-externa-frontend-v2-acesso-mttdg1f9'
FEATURE='acesso'
BASE_BRANCH='spec/acesso'
...
preparar_ambiente() {
  ...
  git ls-files --error-unmatch -- '.spec/features/acesso/spec.md' >/dev/null 2>&1 || falhar "spec.md não está commitada — os worktrees das faixas precisam dela no git"
  ...
  git worktree prune
  LOG_DIR="$(dirname "$TOPLEVEL")/onp-worktrees/api-externa-frontend-v2-acesso-logs"
  WT_BASE="$(dirname "$TOPLEVEL")/onp-worktrees/api-externa-frontend-v2-acesso"
...
mesclar_faixa() { # $1=faixa $2=branch $3=worktree $4=exit-da-faixa
  if [ "$4" -ne 0 ]; then
    ...
    amarelo "  reexecute só ela: bash .spec/features/acesso/executar-tarefas.sh --faixa $1"
...
executar_faixa_1() {
  ...
  preparar_worktree 'faixa-1' 'spec/acesso-faixa-1' "$WT" || return 1
  evento --tipo faixa --faixa 'faixa-1' --estado executando --tentativa "$(tentativa 'faixa-1')"
  : > "$LOG_DIR/faixa-1.log"
  (
    cd "$WT" || exit 9
    rodar_tarefa 'faixa-1' 'T-013' 'Você executa UMA tarefa da feature "acesso" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/acesso/spec.md, .spec/features/acesso/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-013 — "Corrigir o contrato da sessão para o que o backend devolve"
  critérios/refs: AC-046 (A sessão nasce de token, tipo e expiraEm)
  arquivos permitidos (e seus testes): src/app/core/sessao/sessao.model.ts, src/app/core/sessao/sessao.service.ts, src/app/core/sessao/sessao.service.spec.ts, src/app/core/sessao/sessao.guard.spec.ts, src/app/core/sessao/autenticacao.interceptor.spec.ts, src/app/core/erros/erro.interceptor.spec.ts
  mensagem de commit: "T-013 acesso: Corrigir o contrato da sessão para o que o backend devolve"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'claude-sonnet-5' medium
  ...
  local st=$?
  mesclar_faixa 'faixa-1' 'spec/acesso-faixa-1' "$WT" "$st" || return 1
...
executar_faixa_2() {
  ...
  preparar_worktree 'faixa-2' 'spec/acesso-faixa-2' "$WT" || return 1
  evento --tipo faixa --faixa 'faixa-2' --estado executando --tentativa "$(tentativa 'faixa-2')"
  : > "$LOG_DIR/faixa-2.log"
  (
    cd "$WT" || exit 9
    rodar_tarefa 'faixa-2' 'T-014' 'Você executa UMA tarefa da feature "acesso" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/acesso/spec.md, .spec/features/acesso/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-014 — "Serviço de acesso: cadastrar e entrar"
  critérios/refs: AC-025 (Cadastro válido leva ao login com o e-mail preenchido), AC-026 (Cadastrar não entra no sistema), AC-035 (Login válido guarda a sessão e leva ao destino interno), AC-046 (A sessão nasce de token, tipo e expiraEm)
  arquivos permitidos (e seus testes): src/app/features/acesso/acesso.model.ts, src/app/features/acesso/acesso.service.ts, src/app/features/acesso/acesso.service.spec.ts
  mensagem de commit: "T-014 acesso: Serviço de acesso: cadastrar e entrar"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'claude-sonnet-5' medium
  ...
  local st=$?
  mesclar_faixa 'faixa-2' 'spec/acesso-faixa-2' "$WT" "$st" || return 1
...
executar_faixa_3() {
  ...
  preparar_worktree 'faixa-3' 'spec/acesso-faixa-3' "$WT" || return 1
  evento --tipo faixa --faixa 'faixa-3' --estado executando --tentativa "$(tentativa 'faixa-3')"
  : > "$LOG_DIR/faixa-3.log"
  (
    cd "$WT" || exit 9
    rodar_tarefa 'faixa-3' 'T-015' 'Você executa UMA tarefa da feature "acesso" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/acesso/spec.md, .spec/features/acesso/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-015 — "Validação no cliente: CPF, senha e e-mail"
  critérios/refs: AC-027 (CPF fora do formato nem chega ao servidor), AC-028 (Senha fora da política nem chega ao servidor), AC-029 (E-mail fora de formato nem chega ao servidor)
  arquivos permitidos (e seus testes): src/app/features/acesso/validadores.ts, src/app/features/acesso/validadores.spec.ts
  mensagem de commit: "T-015 acesso: Validação no cliente: CPF, senha e e-mail"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'claude-sonnet-5' medium
  ...
  local st=$?
  mesclar_faixa 'faixa-3' 'spec/acesso-faixa-3' "$WT" "$st" || return 1
...
executar_faixa_4() {
  ...
  preparar_worktree 'faixa-4' 'spec/acesso-faixa-4' "$WT" || return 1
  evento --tipo faixa --faixa 'faixa-4' --estado executando --tentativa "$(tentativa 'faixa-4')"
  : > "$LOG_DIR/faixa-4.log"
  (
    cd "$WT" || exit 9
    rodar_tarefa 'faixa-4' 'T-016' 'Você executa UMA tarefa da feature "acesso" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/acesso/spec.md, .spec/features/acesso/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-016 — "Tela de cadastro"
  critérios/refs: AC-025 (Cadastro válido leva ao login com o e-mail preenchido), AC-026 (Cadastrar não entra no sistema), AC-030 (E-mail duplicado preserva o formulário e destaca só o e-mail), AC-031 (CPF duplicado destaca só o CPF), AC-032 (Senha recusada pelo servidor destaca a senha), AC-033 (Erro de validação do servidor se distribui pelos campos), AC-034 (As regras aparecem antes de o investidor errar)
  ...
  mensagem de commit: "T-016 acesso: Tela de cadastro"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'claude-sonnet-5' medium
  ...
  local st=$?
  mesclar_faixa 'faixa-4' 'spec/acesso-faixa-4' "$WT" "$st" || return 1
...
executar_faixa_5() {
  ...
  preparar_worktree 'faixa-5' 'spec/acesso-faixa-5' "$WT" || return 1
  evento --tipo faixa --faixa 'faixa-5' --estado executando --tentativa "$(tentativa 'faixa-5')"
  : > "$LOG_DIR/faixa-5.log"
  (
    cd "$WT" || exit 9
    rodar_tarefa 'faixa-5' 'T-017' 'Você executa UMA tarefa da feature "acesso" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/acesso/spec.md, .spec/features/acesso/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-017 — "Tela de login"
  critérios/refs: AC-035 (Login válido guarda a sessão e leva ao destino interno), AC-036 (Credencial incorreta não revela qual campo errou), AC-037 (O botão fica indisponível durante o envio), AC-038 (A tela de login não oferece recuperação de senha), AC-039 (Quem já tem sessão não fica na tela de login), AC-045 (Sessão vencida durante uma ação explica o motivo no login)
  ...
  mensagem de commit: "T-017 acesso: Tela de login"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'claude-sonnet-5' medium
  ...
  local st=$?
  mesclar_faixa 'faixa-5' 'spec/acesso-faixa-5' "$WT" "$st" || return 1
...
executar_faixa_6() {
  ...
  preparar_worktree 'faixa-6' 'spec/acesso-faixa-6' "$WT" || return 1
  evento --tipo faixa --faixa 'faixa-6' --estado executando --tentativa "$(tentativa 'faixa-6')"
  : > "$LOG_DIR/faixa-6.log"
  (
    cd "$WT" || exit 9
    rodar_tarefa 'faixa-6' 'T-018' 'Você executa UMA tarefa da feature "acesso" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/acesso/spec.md, .spec/features/acesso/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-018 — "Casca da área interna: sair e aviso de sessão acabando"
  critérios/refs: AC-041 (Sair limpa a sessão e leva ao login), AC-043 (O aviso sai do próprio login, cinco minutos antes), AC-044 (O aviso não bloqueia e oferece entrar de novo)
  arquivos permitidos (e seus testes): src/app/layout/casca.ts, src/app/layout/casca.html, src/app/layout/casca.scss, src/app/layout/casca.spec.ts
  mensagem de commit: "T-018 acesso: Casca da área interna: sair e aviso de sessão acabando"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'claude-sonnet-5' medium
  ...
  local st=$?
  mesclar_faixa 'faixa-6' 'spec/acesso-faixa-6' "$WT" "$st" || return 1
...
executar_faixa_7() {
  ...
  preparar_worktree 'faixa-7' 'spec/acesso-faixa-7' "$WT" || return 1
  evento --tipo faixa --faixa 'faixa-7' --estado executando --tentativa "$(tentativa 'faixa-7')"
  : > "$LOG_DIR/faixa-7.log"
  (
    cd "$WT" || exit 9
    rodar_tarefa 'faixa-7' 'T-019' 'Você executa UMA tarefa da feature "acesso" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/acesso/spec.md, .spec/features/acesso/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-019 — "Rotas de acesso e destino mínimo da área interna"
  critérios/refs: AC-039 (Quem já tem sessão não fica na tela de login), AC-040 (Login e cadastro se alcançam em um clique, nos dois sentidos), AC-042 (Depois de sair, voltar pelo navegador não mostra tela interna)
  arquivos permitidos (e seus testes): src/app/app.routes.ts, src/app/features/acesso/acesso.routes.ts, src/app/features/painel/painel-provisorio.ts, src/app/features/acesso/acesso.routes.spec.ts
  mensagem de commit: "T-019 acesso: Rotas de acesso e destino mínimo da área interna"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'claude-sonnet-5' medium
  ...
  local st=$?
  mesclar_faixa 'faixa-7' 'spec/acesso-faixa-7' "$WT" "$st" || return 1
...
encerrar() { # $1=escopo
  ...
  if [ "$COM_GATE" -eq 0 ]; then
    ...
    if [ -z "$FALHAS" ]; then
      ...
      amarelo "  para o veredito: bash .spec/features/acesso/executar-tarefas.sh --gate"
      exit 0
    fi
...
listar() {
  ...
  echo "  faixa-6  onda 2  T-018"
  echo "  faixa-7  onda 3  T-019"
  echo
