#!/usr/bin/env bash
# executar-tarefas.sh — gerado por `onp-spec plano repaginacao-painel` em 2026-09-09 17:53
...
#
# resumo do que está rolando, a qualquer momento: onp-spec resumo repaginacao-painel
set -u
set -o pipefail

RUN_ID='api-externa-frontend-v2-repaginacao-painel-mtuecptx'
FEATURE='repaginacao-painel'
BASE_BRANCH='spec/repaginacao-painel'
...
preparar_ambiente() {
  ...
  git ls-files --error-unmatch -- '.spec/features/repaginacao-painel/spec.md' >/dev/null 2>&1 || falhar "spec.md não está commitada — os worktrees das faixas precisam dela no git"
  ...
  git worktree prune
  LOG_DIR="$(dirname "$TOPLEVEL")/onp-worktrees/api-externa-frontend-v2-repaginacao-painel-logs"
  WT_BASE="$(dirname "$TOPLEVEL")/onp-worktrees/api-externa-frontend-v2-repaginacao-painel"
...
mesclar_faixa() { # $1=faixa $2=branch $3=worktree $4=exit-da-faixa
  if [ "$4" -ne 0 ]; then
    ...
    amarelo "  reexecute só ela: bash .spec/features/repaginacao-painel/executar-tarefas.sh --faixa $1"
...
executar_faixa_1() {
  ...
  preparar_worktree 'faixa-1' 'spec/repaginacao-painel-faixa-1' "$WT" || return 1
  evento --tipo faixa --faixa 'faixa-1' --estado executando --tentativa "$(tentativa 'faixa-1')"
  : > "$LOG_DIR/faixa-1.log"
  (
    cd "$WT" || exit 9
    rodar_tarefa 'faixa-1' 'T-092' 'Você executa UMA tarefa da feature "repaginacao-painel" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/repaginacao-painel/spec.md, .spec/features/repaginacao-painel/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-092 — "Barra de mercado: chips, superfície escura e rolagem"
  critérios/refs: AC-233 (A barra rola para a esquerda, em loop), AC-234 (A rolagem para quando o investidor quer ler), AC-235 (Movimento reduzido desliga a rolagem), AC-236 (A barra não finge ser tempo real), AC-237 (Cada cotação é um chip legível)
  ...
  mensagem de commit: "T-092 repaginacao-painel: Barra de mercado: chips, superfície escura e rolagem"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'claude-sonnet-5' high
  ...
  local st=$?
  mesclar_faixa 'faixa-1' 'spec/repaginacao-painel-faixa-1' "$WT" "$st" || return 1
  marcar_concluidas T-092
  return 0
}
...
executar_faixa_2() {
  ...
  preparar_worktree 'faixa-2' 'spec/repaginacao-painel-faixa-2' "$WT" || return 1
  evento --tipo faixa --faixa 'faixa-2' --estado executando --tentativa "$(tentativa 'faixa-2')"
  : > "$LOG_DIR/faixa-2.log"
  (
    cd "$WT" || exit 9
    rodar_tarefa 'faixa-2' 'T-093' 'Você executa UMA tarefa da feature "repaginacao-painel" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/repaginacao-painel/spec.md, .spec/features/repaginacao-painel/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-093 — "Consolidado com hierarquia de número"
  critérios/refs: AC-238 (O número domina o cartão), AC-239 (Nenhum indicador promete comparação que não existe)
  ...
  mensagem de commit: "T-093 repaginacao-painel: Consolidado com hierarquia de número"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'claude-sonnet-5' medium
  ...
  local st=$?
  mesclar_faixa 'faixa-2' 'spec/repaginacao-painel-faixa-2' "$WT" "$st" || return 1
  marcar_concluidas T-093
  return 0
}
...
executar_faixa_3() {
  ...
  preparar_worktree 'faixa-3' 'spec/repaginacao-painel-faixa-3' "$WT" || return 1
  evento --tipo faixa --faixa 'faixa-3' --estado executando --tentativa "$(tentativa 'faixa-3')"
  : > "$LOG_DIR/faixa-3.log"
  (
    cd "$WT" || exit 9
    rodar_tarefa 'faixa-3' 'T-094' 'Você executa UMA tarefa da feature "repaginacao-painel" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/repaginacao-painel/spec.md, .spec/features/repaginacao-painel/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-094 — "Últimas movimentações com monograma e selo"
  critérios/refs: AC-240 (Movimentação mostra o ativo com monograma)
  ...
  mensagem de commit: "T-094 repaginacao-painel: Últimas movimentações com monograma e selo"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'claude-sonnet-5' medium
  ...
  local st=$?
  mesclar_faixa 'faixa-3' 'spec/repaginacao-painel-faixa-3' "$WT" "$st" || return 1
  marcar_concluidas T-094
  return 0
}
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'claude-sonnet-5' medium
  ...
  local st=$?
  mesclar_faixa 'faixa-4' 'spec/repaginacao-painel-faixa-4' "$WT" "$st" || return 1
...
executar_faixa_5() {
  ...
  preparar_worktree 'faixa-5' 'spec/repaginacao-painel-faixa-5' "$WT" || return 1
  evento --tipo faixa --faixa 'faixa-5' --estado executando --tentativa "$(tentativa 'faixa-5')"
  : > "$LOG_DIR/faixa-5.log"
  (
    cd "$WT" || exit 9
    rodar_tarefa 'faixa-5' 'T-096' 'Você executa UMA tarefa da feature "repaginacao-painel" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/repaginacao-painel/spec.md, .spec/features/repaginacao-painel/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-096 — "Composição da tela do painel"
  critérios/refs: AC-238 (O número domina o cartão)
  arquivos permitidos (e seus testes): src/app/features/painel/painel.html, src/app/features/painel/painel.scss, src/app/features/painel/painel.spec.ts
  mensagem de commit: "T-096 repaginacao-painel: Composição da tela do painel"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'claude-sonnet-5' low
  ...
  local st=$?
  mesclar_faixa 'faixa-5' 'spec/repaginacao-painel-faixa-5' "$WT" "$st" || return 1
...
encerrar() { # $1=escopo
  ...
  if [ "$COM_GATE" -eq 0 ]; then
    ...
    if [ -z "$FALHAS" ]; then
      ...
      amarelo "  para o veredito: bash .spec/features/repaginacao-painel/executar-tarefas.sh --gate"
      exit 0
    fi
...
listar() {
  ...
  echo "  faixa-1  onda 1  T-092"
  echo "  faixa-2  onda 1  T-093"
  echo "  faixa-3  onda 1  T-094"
  echo "  faixa-4  onda 2  T-095"
  echo "  faixa-5  onda 2  T-096"
  echo
